...
class PersonalBackup(WebpageGetter):
    ...
    def _saveResource(self,
        ...
        if date is None:
            pass # date=Now
        _=url,data # TODO: Save this to database
    ...
    def _webfetch(self,url:UrlCompatible)->typing.Tuple[str,MimeType]:
        ...
        returns (url,mime)
        """
        data=[None] # cheat the scope for the inner function
        mime=[None]
        f=self._getFetcher()
        #f.debug=99
    ...
    def get(self,
        ...
        """
        get something from the personal backup
        """
    ...
    def backup(self,website:Website)->None:
        ...
        """
        # This gets us in the ballpark, but usually
        # the MIME type makes the decision
    ...
    def cmdline(self,argv:typing.Optional[typing.Iterable[str]]=None)->int:
        ...
        if len(argv)<2:
            ...
            from pybook import Bookmark # type: ignore # noqa: E501 # pylint: disable=line-too-long,import-error
            for arg in argv[1:]:
                if arg[0]=='-':
                    if arg[1]=='-':
                        arg=arg[2:].split('=',1)
