...
def _createIdexPage(outDir,catalog):
    ...
    html.append('\t\t.lesson {width:120;height:200;float:left;display:inline-block;margin:3px;background:#888888}')
    ...
    for cat in catalog:
        ...
        html.append('\t\t<div class="lesson"><a href="'+cat[2]+'"><img src="'+cat[1]+'" /><br />'+cat[0]+'</a></div>')
    html.append('\t</div>')
    html.append('</body>')
    html.append('</html>')
    f=open(outDir+os.sep+'index.html','w')
    f.write('\n'.join(html))
    f.close()
...
def __youtubeFetch(counter,wf,idOrUrl,filename,mediaFormat=defaultMediaFormat,outDir='.'):
    ...
    def gotVideoPage(url,data):
        ...
        data=data[-1]
        flashvars={}
        for d in data.split('flashvars="',1)[-1].split('"',1)[0].split('&amp;'):
            d=d.split('=',1)
            flashvars[d[0]]=d[-1]
            #EXAMPLE FLASHVARS:
            #------------------
            #ttsurl=http%3A%2F%2Fwww.youtube.com%2Fapi%2Ftimedtext%3Fsparams%3Dasr_langs%252Ccaps%252Cexpire%252Cv%26asr_langs%3Den%252Cja%26caps%3Dasr%26expire%3D1317358800%26key%3Dyttt1%26signature%3DDDA04BEEB92EA496E656DE92F9C38DEE5D8841EC.036542E5C4E031ABD6422E77E10C19BA7A856131%26hl%3Den
            ...
            #sffb=True
        video_id=urllib.unquote(flashvars['video_id'])
        t=urllib.unquote(flashvars['t'])
        downloadUrl='http://www.youtube.com/get_video?video_id='+video_id+'&t='+t
...
def _cheaterDownload(video_id):
    ...
    try:
        ...
        cmd='wget --no-check-certificate -O - https://github.com/JakeWharton/py-videodownloader/tarball/master | tar -xzf -'
        os.system(cmd)
...
def _cheaterDownload2(video_id):
    ...
    try:
        ...
        print('***************************************')
        print('  NEED TO INSTALL YOUTUBE DOWNLOADER!')
        cmd='wget -O Sources.zip http://py-youtube-downloader.googlecode.com/files/Sources.zip;unzip -q Sources.zip;rm -rf Sources.zip py2exe'
        os.system(cmd)
        import youtube as videodownloader
...
def _getYoutubeDownloadUrl(idOrUrl,mediaFormat=defaultMediaFormat):
    ...
    idOrUrl=getId(idOrUrl)
    url='http://www.youtube.com/get_video?fmt='+str(mediaFormat)+'&video_id='+idOrUrl
    return url
