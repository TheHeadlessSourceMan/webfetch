...
class WebSearch:
	...
	def _googleTranslate(self,text:str,fromLanguage:str,toLanguage:str='en')->typing.Any:
		url='http://translate.google.com/translate_t'
		url=url+'?sl='+fromLanguage+'&tl='+toLanguage
		url=url+'&hl=en&ie=UTF8'
		url=url+'&text='+urllib.parse.quote_plus(text)
		soup=self._url2soup(self,url)
		return soup('div',id='result_box')[0].string

	def _googleSearch(self,searchString:str,page:int=1,resultsPerPage:int=50)->typing.Any:
		retval=[]
	...
	def _yahooSearch(self,searchString:str)->typing.Any:
		...
		urlRe=re.compile(r'/\*\*(.*)')
		try:
			return [urllib.parse.unquote_plus(urlRe.findall(h3.find('a')['href'])[0]) for h3 in soup.find('div',id='web').findAll('h3')]
		except Exception as e:
