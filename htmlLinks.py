...
class HtmlLinks:
	...
	def _guessMime(self,url):
		...
		"""
		url=url.rsplit('/',1)[-1]
		if url.find('&')>=0 or url.find('.')<0: # generated webpages are inconclusive
			return ''
		extn=url.rsplit('.',1)[-1]
		if extn in ['','php','jsp','asp','cgi']: # generated webpages are inconclusive
			return ''
	...
	def getByMime(self,data,mime,basePath=None,includeUnknownTypes=False):
		...
		if mime.startsWith('image'):
			for i in self.getImages(data,basePath):
				if mime=='image' or self._guessMime(a) in mimes:
					urls.append(i)
			...
			if mime=='application/javascript' or mime=='application/emcascript':
	...
	def getATags(self,data,basePath=None):
		...
		if HtmlLinks.__aTagRegex is None:
			HtmlLinks.__aTagRegex=re.compile(r"""<a\s+(?P<attrs>.*?href\s*=\s*"(?P<url>[^"]*)"[^>]*)""",re.IGNORECASE|re.MULTILINE)
	...
	def getSimpleUrlsFromJsCode(self,data,basePath=None,includeLinkLikeStrings=True):
		"""
		...
		will not be added.  For instance:
			...
			document.location="foo.com/item"+a+".gif" // not ok.  don't know what that variable would be
			
		includeLinkLikeStrings tells us whether or not to search for things that look like
		possible links.  For example:
			var x = "http://whatever";
		...
		if HtmlLinks.__jsDocLocationRegex is None:
			HtmlLinks.__jsDocLocationRegex=re.compile(r"""[.\s]location\s*=\s*(?P<quote>['"]*)(?P<url>.+?)(?P=quote)(?P<compounded>\s*[+])?""")
		if HtmlLinks.__jsWindowOpen is None:
			HtmlLinks.__jsWindowOpen=re.compile(r"""window[.]open\s*[(]\s*(?P<quote>['"]+)(?P<url>.+?)(?P=quote)(?P<compounded>\s*[+])?""")
		if includeLinkLikeStrings and HtmlLinks.__jsLinkStrings is None:
			HtmlLinks.__jsLinkStrings=re.compile(r"""(?P<quote>['"]+)(?P<url>http(?:s)?[:][/].+?)(?P=quote)(?P<compounded>\s*[+])?""")
		# search in the <script> blocks
		if HtmlLinks.__scriptTagRegex is None:
			HtmlLinks.__scriptTagRegex=re.compile(r"""<script[^>]*?>(?P<code>.*?)</script>""",re.IGNORECASE|re.MULTILINE|re.DOTALL)
		...
		if HtmlLinks.__onclickAttrRegex is None:
			HtmlLinks.__onclickAttrRegex=re.compile(r"""<.+?\s+onclick\s*=\s*["](?P<code>.*?)["].*?>""",re.IGNORECASE|re.MULTILINE|re.DOTALL)
	...
	def getImages(self,data,basePath=None):
		...
		if HtmlLinks.__imgTagRegex is None:
			HtmlLinks.__imgTagRegex=re.compile(r"""<img\s+(?P<attrs>.*?src\s*=\s*"(?P<url>[^"]*)"[^>]*)""",re.IGNORECASE|re.MULTILINE)
		for match in HtmlLinks.__imgTagRegex.finditer(data):
			url=self._fixurl(match.group('url'),basePath)
			urls.append(url)
		return urls
	...
	def getLinkRels(self,data,basePath=None):
		...
		if HtmlLinks.__linkRelTagRegex is None:
			HtmlLinks.__linkRelTagRegex=re.compile(r"""<link\s+(?P<attrs>rel="(?P<rel>[^"]*)".*?href\s*=\s*"(?P<url>[^"]*)"[^>]*)""",re.IGNORECASE|re.MULTILINE)
	...
	def getEmbedded(self,data,basePath=None):
		"""
		Gets all urls pointed to by <embed src=""> or <object data=""> or <applet code=""> tags
		NOTE: This will only return one item of each url, for pages that contain both types of embedding
		...
		if HtmlLinks.__embedTagRegex is None:
			HtmlLinks.__embedTagRegex=re.compile(r"""<embed\s+(?P<attrs>type="(?P<mimetype>[^"]*)".*?src\s*=\s*"(?P<url>[^"]*)"[^>]*)""",re.IGNORECASE|re.MULTILINE)
		...
		if HtmlLinks.__objectTagRegex is None:
			HtmlLinks.__objectTagRegex=re.compile(r"""<object\s+(?P<attrs>type="(?P<mimetype>[^"]*)".*?data\s*=\s*"(?P<url>[^"]*)"[^>]*)""",re.IGNORECASE|re.MULTILINE)
		for match in HtmlLinks.__objectTagRegex.finditer(data):
			url=self._fixurl(match.group('url'),basePath)
			embeds[url]=(match.group('mimetype').lower(),url)
		if HtmlLinks.__objectTagRegex is None:
			HtmlLinks.__objectTagRegex=re.compile(r"""<applet\s+(?P<attrs>codebase="(?P<codebase>[^"]*)".*?code\s*=\s*"(?P<url>[^"]*)"[^>]*)""",re.IGNORECASE|re.MULTILINE)
		for match in HtmlLinks.__objectTagRegex.finditer(data):
