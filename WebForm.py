...
formExpr=re.compile(
    """<form(?P<attributes>[^>]*)>(?P<contents>.*?)</form>""",
    re.IGNORECASE|re.DOTALL)
formElementExpr=re.compile(
    r"""<(?P<tagName>[^/][^\s>]*)(?P<attributes>([^/>])*)""")
attributeExpr=re.compile(
...
class WebForm:
    ...
    HINT: To get ALL forms on an html page
    use the global function getForms(html)
    """
    ...
    def setElements(self,
        ...
        for k,v in list(toThis.items()):
            self.setElement(k,v,createIfMissing)
    ...
    def getUrl(self)->Url:
        ...
        if 'action' in self.attributes:
            ...
            pos=url.find('://')
            if self.baseUrl is not None and (pos is None or pos>7 or pos<3): # complete relative url  # noqa: E501 # pylint: disable=line-too-long
                url=url.strip()
                if url[0]=='/':
                    url=url[1:]
    ...
    def currentCgi(self)->str:
        ...
        for k,v in list(self.elements.items()):
            ...
            cgi=cgi+combiner+urllib.parse.quote_plus(k)+'='+urllib.parse.quote_plus(v) # noqa: E501 # pylint: disable=line-too-long
            combiner='&'
        return cgi
...
if __name__ == '__main__':
    ...
    if err:
        print('USAGE: WebForm.py [-s,-h,-c,-r] "url or text" [name=value] [...]') # noqa: E501 # pylint: disable=line-too-long
        print('    -h ........... return the form as html')
        print('    -c ........... return the cgi string for sumitting the form') # noqa: E501 # pylint: disable=line-too-long
