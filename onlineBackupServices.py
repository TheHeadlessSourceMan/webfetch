...
class OnlineBackupService(WebpageGetter):
    ...
    def __init__(self,xmlConfig=None):
        """
        xmlConfig is a pig-xml.  It must all be in the
        same order as expeceted or... kaboom!
        ...
        if xmlConfig is not None:
            xmlConfig=xmlConfig.strip()\
                .rsplit('</OnlineBackupService',1)[0]\
                .split(' ',1)[-1]
...
class OnlineBackupServices(WebpageGetter):
    """
    A registry of common online backup services
    (such as wayback)
    """
    def __init__(self):
        WebpageGetter.__init__(self)
        self._services={}
        filename=(os.path.abspath(__file__).rsplit(os.sep,1)[0])+os.sep+'OnlineBackupServices.xml' # noqa: E501 # pylint: disable=line-too-long
        self.load(filename)

    def load(self,filename):
        """
        Load file containing info about this backup service
        """
    ...
    def get(self,url,date=None,servicePreference=None):
        ...
        elif not type(servicePreference,list):
        ...
        for service in servicePreference:
            ...
            if date is None or service.canGetByDate:
                ...
                if result is not None:
                    return result
        if date is not None:
            # we didn't find anything in the date-matching OnlineBackupServices
            # so lets try the others
            for service in servicePreference:
    ...
    def cmdline(self,argv=None):
        ...
        if printhelp:
            ...
            print('OPTIONS:')
            print('    --service[=name] ........ the service to try (default=all)') # noqa: E501 # pylint: disable=line-too-long
