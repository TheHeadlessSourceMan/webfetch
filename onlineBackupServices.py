...
class OnlineBackupService(WebpageGetter):
	...
	def __init__(self,xmlConfig=None):
		"""
		xmlConfig is a pig-xml.  It must all be in the same order as expeceted or... kaboom!
		...
		if xmlConfig is not None:
			xmlConfig=xmlConfig.strip().rsplit('</OnlineBackupService',1)[0].split(' ',1)[-1]
			self.description=xmlConfig.rsplit('>',1)[-1].strip()
			xmlConfig=xmlConfig.split('>',1)[0].split('"') # [name=][name][ href=][href]
...
class OnlineBackupServices(WebpageGetter):
	def __init__(self):
		WebpageGetter.__init__(self)
		self._services={}
		filename=(os.path.abspath(__file__).rsplit(os.sep,1)[0])+os.sep+'OnlineBackupServices.xml'
		self.load(filename)
	...
	def get(self,url,date=None,servicePreference=None):
		...
		elif type(servicePreference)!=list:
		...
		for service in servicePreference:
			...
			if date is None or service.canGetByDate:
				...
				if result is not None:
					return result
		if date is not None:
			# we didn't find anything in the date-matching OnlineBackupServices, so lets try the others
			for service in servicePreference:
		...
		if len(argv)<2:
			...
			for arg in argv[1:]:
				if arg[0]=='-':
					...
					if service=='all':
						service=None
					elif service not in list(self._services.keys()):
						print('ERR: Unknown service "'+service+'"')
						printhelp=True
					else:
						stuff=self.get(url,date,service)
						url=url.replace('/','_SLASH_').replace(':','_COLON_').replace('.','_DOT_')
