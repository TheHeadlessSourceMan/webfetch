...
class WebFetchPlus:
    ...
    def __init__(self,
        personalBackups:typing.Union[
            None,str,PersonalBackup,typing.Iterable[PersonalBackup]]=None,
        ...
        if useOnlineBackupServices is not None \
            and not useOnlineBackupServices:
            #
    ...
    def get(self,
        ...
        :websites: if it is a url, will autocreate a website object for
            fetching a single html page with images and css only.
            The new object will be returned.
            if it is an array, just to make it so you don't have to
                loop externally
        """
        if isinstance(websites,list):
            ...
            for website in websites:
                results.append(self.get(
                    website,date,fetchIfMissing,addIfFetched))
            return results
    ...
    def getImages(self,
        ...
        :websites: if it is a url then it will just fetch the images from there
            if it is a websites object, it will extract the url from that and
                create a new website object (leaving the original intact)
            if it is an array, just to make it so you don't have to
                loop externally
        """
        if isinstance(websites,list):
    ...
    def getAudio(self,
        ...
        :websites: if it is a url then it will just fetch the audio from there
            if it is a websites object, it will extract the url from that and
                create a new website object (leaving the original intact)
            if it is an array, just to make it so you don't have
                to loop externally
        """
        if isinstance(websites,list):
            results=[]
            for website in websites:
                results.append(self.getAudio(
                    website,date,fetchIfMissing,addIfFetched))
            return results
        if not isinstance(websites,str):
            website=Website(websites.url)
    ...
    def getHtmlOnly(self,
        ...
        :websites: if it is a url then it will just fetch the html from there,
            if it is a websites object, it will extract the url from that and
                create a new website object (leaving the original intact)
            if it is an array, just to make it so you don't have to
                loop externally
        ...
        if isinstance(websites,list):
            results=[]
            for website in websites:
                results.append(self.getHtmlOnly(
                    website,date,fetchIfMissing,addIfFetched))
            return results
        if not isinstance(websites,str):
            website=Website(websites.url)
