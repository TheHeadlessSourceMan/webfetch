...
class WebFetchPlus:
    ...
    """
    def __init__(self,
        personalBackups:typing.Union[
            None,
            str,
            PersonalBackup,typing.Iterable[PersonalBackup]]=None,
        useOnlineBackupServices:typing.Union[bool,str]=True):
        """
        :personalBackups: can be PersonalBackup object, string database name,
            or any array thereof
        :useOnlineBackupServices: can be True,False, or an xml filename to
            the list of backup services to use
        """
        self._personalBackups:typing.List[PersonalBackup]=[]
        self._webpageGetters:typing.List[WebpageGetter]=[]
        # add all personalBackups
        if isinstance(personalBackups,list):
            personalBackups=[]
        ...
        for pb in self._personalBackups: # first check the personal backup system(s) # noqa: E501 # pylint: disable=line-too-long
            self._webpageGetters.append(pb)
        from .internet import Internet
        i=Internet()
        self._webpageGetters.append(i)
        if useOnlineBackupServices is not None and useOnlineBackupServices:
    ...
    def get(self,
        ...
        if websites is a url, will autocreate a website object for fetching
        a single html page with images and css only.  The new object will be
        returned.

        websites can also be an array, just to make it
        so you don't have to loop externally
        """
        if isinstance(websites,list):
            results=[]
            for website in websites:
                results.append(
                    self.get(website,date,fetchIfMissing,addIfFetched))
            return results
    ...
    def getImages(self,
        ...
        if websites is a url then it will just fetch the images from there,
        otherwise if it is a websites object, it will extract the url from
        that and create a new website object (leaving the original intact)

        websites can also be an array, just to make it so you
        don't have to loop externally
        """
        if isinstance(websites,list):
            results=[]
            for website in websites:
                results.append(
                    self.getImages(website,date,fetchIfMissing,addIfFetched))
            return results
        if not isinstance(websites,str):
    ...
    def getAudio(self,
        ...
        if websites is a url then it will just fetch the audio from there,
        otherwise if it is a websites object, it will extract the url from that
        and create a new website object (leaving the original intact)
        ...
        if isinstance(websites,list):
            ...
            for website in websites:
                results.append(
                    self.getAudio(website,date,fetchIfMissing,addIfFetched))
            return results
        if not isinstance(websites,str):
            website=Website(websites.url)
        else:
    ...
    def getHtmlOnly(self,
        ...
        if websites is a url then it will just fetch the html from there,
        otherwise if it is a websites object, it will extract the url from that
        and create a new website object (leaving the original intact)
        ...
        if isinstance(websites,list):
            ...
            for website in websites:
                results.append(
                    self.getHtmlOnly(website,date,fetchIfMissing,addIfFetched))
            return results
        if not isinstance(websites,str):
            website=Website(websites.url)
        else:
