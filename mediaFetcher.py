...
class Download:
    ...
    def __init__(self,downloadTo:str=None):
        ...
        If None, will search for known media types.  If an array, it will
        search in the first url for match like the first element, then from
        the pages loaded from each of those search for match like
        the second element, and so on.
        (Any array element can be None as well.)

        downloadTo is the directory to put files in. It will be created if
        it does not exist.
        If None, will create a directory with the current date.
        """
        self.name:str=''
    ...
    def load(self,node)->"Download":
        ...
        for url in node.getElementsByTagName('urlLike'):
            try:
                self.urlLike.append(re.compile(url.firstChild.wholeText,flags=re.IGNORECASE|re.DOTALL)) # noqa: E501 # pylint: disable=line-too-long
        ...
        #    urlLike=[re.compile("""\"([^"]+?.(?:"""+'|'.join(self.mediaTypes)+""")[^"]*?)\"""",flags=re.IGNORECASE|re.DOTALL)] # noqa: E501 # pylint: disable=line-too-long
        ...
        #    for u in urlLike:
        #        if u is None:
        #            u2.append(re.compile("""\"([^"]+?.(?:"""+'|'.join(self.mediaTypes)+""")[^"]*?)\""""),flags=re.IGNORECASE|re.DOTALL) # noqa: E501 # pylint: disable=line-too-long
        #        else:
...
class MediaFetcher(WebFetch):
    ...
    def getFlashMedia(self,
        ...
        regex="""\0([^\0]+?.(?:"""+'|'.join(self.mediaTypes)+"""))\0"""
        mediaFinder=re.compile(regex)
    ...
    def _startFound(self,baseUrl:UrlCompatible,html:str)->None:
        ...
        for match in download.urlLike[0].finditer(html):
            ...
            try:
                ...
                raise IndexError(f'No groups in expression for search: {download.name}') from e # noqa: E501 # pylint: disable=line-too-long
            if len(download.urlLike)>1:
                ...
                if url in self.downloadRecords:
                    ...
                    self.links[f'[old] {filename}']=\
                        f'<b><a href="{self.downloadRecords[url]}">[old] {filename}</a></b>' # noqa: E501 # pylint: disable=line-too-long
                    if self.debug>=2:
