...
If it appears to be generated by javascript, will get using browser via selenium.
"""
import typing
import os
import sys
...
class WebFetch:
	...
	def __init__(self,
		...
		"""
		If remoteIp is unspecified, will automatically create a local selenium server
		and attach to it
		...
		self.trickyPages:typing.List[str]=[] # Keep track of pages that employ javascript tricks like document.write(html)
		if startSeleniumNow:
			self._start()
	...
	def _start(self,restartIfRunning:bool=True)->None:
		...
		if self.p is None:
			...
			jar=os.sep.join([directory,'selenium-server-'+self.seleniumVersion,'selenium-server.jar'])
			pythonDir=os.sep.join([directory,'selenium-python-client-driver-'+self.seleniumVersion])
	...
	def runNext(self)->None:
		...
		if urlDenotationPos<3 or urlDenotationPos>7:
			...
			if not returnSeleniumObject:
				# TODO: Somehow turn this into a selenium.selenium object and call fn
				retval=''
			else:
				retval=fn('',html)
			...
			if f is not None:
				...
				if self.noSelenium:
					...
					if hasJavascriptTricksRE is None:
						import re
						hasJavascriptTricksRE=r"""(innerhtml\s*=)|(document[.]write\s*\()|(<script[^>]*src="[^"]+"[^>]*>)"""
						hasJavascriptTricksRE=re.compile(hasJavascriptTricksRE,re.IGNORECASE)
					if (not self.failoverOnGeneratedPages) or hasJavascriptTricksRE.search(html) is None:
						retval=fn(url,html)
					else:
						# There were JavaScript tricks employed.  Must use Selenium to decode page.
						if self.debug>=2:
							print('There were JavaScript tricks employed on this page.')
